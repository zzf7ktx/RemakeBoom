...
int game::menuLoop()
{
	...
	while (true)
	{
		...
		if (GetAsyncKeyState(key[0]) || GetAsyncKeyState(key[5]))
		{
			chon--;
			if (chon == 0)
			{
		...
		else if (GetAsyncKeyState(key[1]) || GetAsyncKeyState(key[6]))
		{	
			chon++;
			if (chon == 0)
			{
		...
		else if (GetAsyncKeyState(key[4]) || GetAsyncKeyState(key[9]))
		{
			if (chon == 1)
			{
				exit(0);
...
void game::chooseMap() 
{
	...
	while (true)
	{
		...
		drawBmp(50 + 300*2, 30 + 150 * 2, mapViewer[c % 8]);
